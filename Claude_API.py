...
def request_gpt_labeling(review_text):
    """단일 리뷰 텍스트를 Claude API에 던져 ABSA 초안 JSON을 받아오는 함수"""
    # Claude는 대화 기록(messages) 안에 system 역할을 넣으면 에러가 납니다. 따로 분리해야 합니다.
    messages = []
    messages.extend(FEW_SHOT_EXAMPLES)
    messages.append({"role": "user", "content": review_text})
    
    try:
        # Claude API 요청 방식 양식으로 수정
        response = client.messages.create(
            model="claude-haiku-4-5",
            max_tokens=2048,      # Claude 작동에 필수적인 인자입니다.
            system=SYSTEM_PROMPT,  # 시스템 프롬프트는 여기에 넣습니다.
            messages=messages,
            temperature=0.0,      
        )

        # Claude 답변 텍스트 추출 방식 적용 및 앞뒤 공백 제거
        raw_content = response.content[0].text.strip()
        
        # 만약 앞뒤에 마크다운 ```json 기호가 붙어있다면 알맹이만 쏙 빼내기
        if raw_content.startswith("```json"):
            raw_content = raw_content.split("```json")[1].split("```")[0].strip()
        elif raw_content.startswith("```"):
            raw_content = raw_content.split("```")[1].split("```")[0].strip()
        
        # [💡 중요 버그 수정]: response... 가 아닌 마크다운을 벗겨낸 raw_content를 파싱하도록 수정했습니다.
        result_json = json.loads(raw_content)
        
        # 만약 결과가 단일 오브젝트 구조로 감쌌을 경우를 대비한 안전 장치
...
if __name__ == "__main__":
    ...
    # 최종 데이터셋 파일 저장 및 구조 확인
    output_path = "./steam_reviews_pre_labeled_claude.jsonl"
