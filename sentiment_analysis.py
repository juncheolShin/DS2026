...
def parse_args():
    ...
    parser.add_argument(
        "--train_data", type=str, default="../data/steam_reviews_pre_labeled_GPT.jsonl",
        help="train file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--base_model", type=str, default="beomi/KcELECTRA-base"
    )
    parser.add_argument(
...
class KcElectraClassifier(nn.Module): # 클래스명도 직관적으로 변경 추천
    def __init__(self, args, num_label, len_tokenizer):
        super(KcElectraClassifier, self).__init__()

        self.num_label = num_label
        # XLMRobertaModel 대신 AutoModel 사용
        self.underlying_model = AutoModel.from_pretrained(args.base_model)
        self.underlying_model.resize_token_embeddings(len_tokenizer)

        self.labels_classifier = SimpleClassifier(args, self.num_label)

    # forward 함수에서 token_type_ids를 받을 수 있도록 수정
    def forward(self, input_ids, attention_mask, token_type_ids=None, labels=None):
        outputs = self.underlying_model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids  # None 고정 대신 입력받은 값 전달
        )
    ...
    for pair in entity_property_pair:
        ...
        for annotation in annotations:
            ...
            if entity_property == pair:
                ...
                entity_property_data_dict['token_type_ids'].append(tokenized_data['token_type_ids'])  # ➕ 추가
                entity_property_data_dict['label'].append(label_name_to_id['True'])

                polarity_data_dict['input_ids'].append(tokenized_data['input_ids'])
                polarity_data_dict['attention_mask'].append(tokenized_data['attention_mask'])
                polarity_data_dict['token_type_ids'].append(tokenized_data['token_type_ids'])  # ➕ 추가
...
def get_dataset(raw_data, tokenizer, max_len):
    input_ids_list = []
    attention_mask_list = []
    token_type_ids_list = []  # ➕ 추가
    token_labels_list = []

    polarity_input_ids_list = []
    polarity_attention_mask_list = []
    polarity_token_type_ids_list = []  # ➕ 추가
    ...
    for utterance in raw_data:
        ...
        token_type_ids_list.extend(entity_property_data_dict['token_type_ids'])  # ➕ 추가
        token_labels_list.extend(entity_property_data_dict['label'])

        polarity_input_ids_list.extend(polarity_data_dict['input_ids'])
        polarity_attention_mask_list.extend(polarity_data_dict['attention_mask'])
        polarity_token_type_ids_list.extend(polarity_data_dict['token_type_ids'])  # ➕ 추가
        polarity_token_labels_list.extend(polarity_data_dict['label'])

    print('polarity_data_count: ', polarity_count)
    print('entity_property_data_count: ', entity_property_count)

    return TensorDataset(torch.tensor(input_ids_list), torch.tensor(attention_mask_list), torch.tensor(token_type_ids_list), torch.tensor(token_labels_list)), \
           TensorDataset(torch.tensor(polarity_input_ids_list), torch.tensor(polarity_attention_mask_list), torch.tensor(polarity_token_type_ids_list), torch.tensor(polarity_token_labels_list))
...
def train_sentiment_analysis(args=None):
    ...
    print('loading model')
    entity_property_model = KcElectraClassifier(args, len(label_id_to_name), len(tokenizer))
    entity_property_model.to(device)

    polarity_model = KcElectraClassifier(args, len(polarity_id_to_name), len(tokenizer))
    ...
    for _ in trange(epochs, desc="Epoch"):
        ...
        for step, batch in enumerate(entity_property_train_dataloader):
            ...
            loss, _ = entity_property_model(b_input_ids, b_input_mask, b_token_type_ids, b_labels)
        ...
        model_saved_path = args.entity_property_model_path + 'entity_model_epoch_' + str(epoch_step) + '.pt'
        ...
        if args.do_eval:
            ...
            for batch in entity_property_dev_dataloader:
                ...
                with torch.no_grad():
                    loss, logits = entity_property_model(b_input_ids, b_input_mask, b_token_type_ids, b_labels)
        ...
        for step, batch in enumerate(polarity_train_dataloader):
            ...
            loss, _ = polarity_model(b_input_ids, b_input_mask, b_token_type_ids, b_labels)
        ...
        model_saved_path = args.polarity_model_path + 'polarity_model_epoch_' + str(epoch_step) + '.pt'
        ...
        if args.do_eval:
            ...
            for batch in polarity_dev_dataloader:
                ...
                with torch.no_grad():
                    loss, logits = polarity_model(b_input_ids, b_input_mask, b_token_type_ids, b_labels)
...
def predict_from_korean_form(tokenizer, ce_model, pc_model, data):
    ...
    for sentence in data:
        ...
        for pair in entity_property_pair:
            ...
            token_type_ids = torch.tensor([tokenized_data['token_type_ids']]).to(device)  # ➕ 추가

            with torch.no_grad():
                _, ce_logits = ce_model(input_ids, attention_mask, token_type_ids)  # ➕ 전달
            ...
            if ce_result == 'True':
                with torch.no_grad():
                    _, pc_logits = pc_model(input_ids, attention_mask, token_type_ids)  # ➕ 전달
...
def test_sentiment_analysis(args):
    ...
    model = KcElectraClassifier(args, len(label_id_to_name), len(tokenizer))
    ...
    for batch in polarity_test_dataloader:
        batch = tuple(t.to(device) for t in batch)
        b_input_ids, b_input_mask, b_token_type_ids, b_labels = batch  # ➕ 변경
        with torch.no_grad():
            loss, logits = polarity_model(b_input_ids, b_input_mask, b_token_type_ids, b_labels)  # ➕ 변경
